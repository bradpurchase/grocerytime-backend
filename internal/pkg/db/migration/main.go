...
func AutoMigrateService(db *gorm.DB) error {
	...
	m = gormigrate.New(db, gormigrate.DefaultOptions, []*gormigrate.Migration{
		...
		},
		{
			...
			Migrate: func(tx *gorm.DB) error {
				return tx.Exec("CREATE INDEX IF NOT EXISTS idx_items_name_grocery_trip_id ON items (name, grocery_trip_id)").Error
			},
			Rollback: func(tx *gorm.DB) error {
		...
		},
		{
			...
			Migrate: func(tx *gorm.DB) error {
				return tx.Exec("CREATE INDEX IF NOT EXISTS idx_meals_date ON meals (date)").Error
			},
			Rollback: func(tx *gorm.DB) error {
				return tx.Exec("DROP INDEX idx_meals_date").Error
			},
		},
		{
