...
func UpdateTrip(args map[string]interface{}) (interface{}, error) {
	...
	if trip.Completed {
		...
		if err := db.Manager.Create(&newTrip).Error; err != nil {
			return trip, err
		}
		...
		if trip.CopyRemainingItems {
			...
			if err := db.Manager.Where("grocery_trip_id = ? AND completed = ?", trip.ID, false).Find(&remainingItems).Error; err != nil {
				return trip, err
			}
			...
			if err := db.Manager.Create(&newItems).Error; err != nil {
				return trip, err
			}
		}
