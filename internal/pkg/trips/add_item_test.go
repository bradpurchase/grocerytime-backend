...
func (s *Suite) TestAddItem_AddsItemToTripWithCategoryName() {
	...
	s.mock.ExpectQuery("^INSERT INTO \"items\" (.+)$").
		WithArgs(trip.ID, sqlmock.AnyArg(), userID, nil, "Apples", 5, false, 1, AnyTime{}, AnyTime{}, nil).
...
func (s *Suite) TestAddItem_NoQuantityArg() {
	...
	itemID := uuid.NewV4()
	s.mock.ExpectQuery("^INSERT INTO \"items\" (.+)$").
		WithArgs(trip.ID, sqlmock.AnyArg(), userID, nil, "Kleenex", 1, false, 1, AnyTime{}, AnyTime{}, nil).
		WillReturnRows(sqlmock.NewRows([]string{"id"}).AddRow(itemID))
...
func (s *Suite) TestAddItem_InlineQuantityInItemName() {
	...
	itemID := uuid.NewV4()
	s.mock.ExpectQuery("^INSERT INTO \"items\" (.+)$").
		WithArgs(trip.ID, sqlmock.AnyArg(), userID, nil, "Apples", 6, false, 1, AnyTime{}, AnyTime{}, nil).
		WillReturnRows(sqlmock.NewRows([]string{"id"}).AddRow(itemID))
